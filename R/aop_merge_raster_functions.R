...
checkdpID <- function(dpID) {
  if (!(dpID %in% lookupTable$dpID)) {
    print('The dpID is not valid. Please select one of the following dpIDs:\n')
...
makeFullSiteMosaics <- function(dpID,year,siteCode,dataRootDir,outFileDir = NULL,apiToken=NULL) {
  ...
  print('Generating full-site mosaic(s)\n')
  print(paste0('dpID: ',dpID,'\n'))
  print(paste0('year: ',year,'\n'))
  print(paste0('site: ',siteCode,'\n'))
  ...
  ## Check if the tiles are already there
  dldirs = getDownloadDirs(dataRootDir, siteCode, dpID, year)
  dataDir = getDataDirs(dataRootDir, siteCode, dpID, dldirs)
  tiles = list.files(dataDir, getDataExts(dpID))
  if(length(tiles) == 0) {
    if (is.null(apiToken)) {
  ...
  ## dataTiles is a list of 1 element and the following for loop doesn't work
  ## make dataTiles a vector
  ## dataTiles = unlist(dataTiles)

  print('Creating full site mosaics/n')
  fullMosaics <- list()
  fullMosaicNames <- list()
  for (i in length(dataTiles)) {
    print(paste0("Processing file # ", i, " : ", basename(dataTiles[[i]])))
  ...
  print('Writing full mosaics to Geotiffs and Cloud-Optimized Geotiffs\n')
  if(is.null(outFileDir)) outFileDir = file.path(dataDir, "../Mosaic")
  if(!dir.exists(outFileDir)) makeDir(outFileDir)

  for (i in 1:length(fullMosaics)) {
    outFileTif <- fullMosaicNames[[i]]
    print(paste0('Generating ',outFileTif,'\n'))
    ...
    print(paste0('Generating ',outFileCog,'\n'))
